...
import { createCommentSchema, createPostSchema, paginationSchema, type Comment, type PaginatedResponse, type Post, type SuccessResponse } from "@/shared/types";
...
export const postRouter = new Hono<Context>().post("/", loggedIn, zValidator("form", createPostSchema), async (c) => {
    ...
    const { title, content, url } = c.req.valid("form") as {
        title: string;
        content: string;
        url: string;
    }
    ...
    const sortByOrder = order === "desc" ? desc(sortByColumn) : asc(sortByColumn);
    ...
    async (c) => {
        ...
        const [count] = await db
            ...
            .where(
                and(
                    eq(commentsTable.postId, String(id)),
